...
th_results_dir = os.path.join('./CSGNet/DAVIS', f'results{num_frames}_th{th}')

scene_list = os.listdir(results_dir)
for scene in scene_list:
    print(f'\n->>> {scene}')
    save_th_mask_to_dir = os.path.join(th_results_dir, scene)
    if not os.path.exists(save_th_mask_to_dir):
        os.makedirs(save_th_mask_to_dir)
        
    scene_mask_path = os.path.join(results_dir, scene)
    ...
    if(results[0]): # if large numbers of frames, slice it
        for idx in results[1]: # for each slice
            ...
            mask_slice = mask_slice.reshape([shape[0], shape[1], shape[2], shape[3]])

            prev = 0
            print('\n\t- Saving thresholding masks:')
            for i in range(shape[0]):
                fname = os.path.basename(slice_X_list[i])
                save_img(os.path.join(save_th_mask_to_dir, fname), mask_slice[i])
                sys.stdout.write('\b' * prev)
                sys.stdout.write('\r')
                s = str(i+1)
                sys.stdout.write(s)
        ...
        mask = mask.reshape([shape[0], shape[1], shape[2], shape[3]])
        
        prev = 0
        print('\n\t- Saving thresholding masks:')
        for i in range(shape[0]):
            fname = os.path.basename(X_list[i])
            save_img(os.path.join(save_th_mask_to_dir, fname), mask[i])
            sys.stdout.write('\b' * prev)
            sys.stdout.write('\r')
            s = str(i+1)
            sys.stdout.write(s)
